...
const prefixClsCrm = prefix + 'modal-confirm'
    ...
    const Instance = new Vue({
        ...
        render (h) {
            ...
            if (this.render) {
                bodyRender = h('div', {
                    attrs: {
                        class: `${prefixClsCrm}-body ${prefixClsCrm}-body-render`
                    }
                }, [this.render(h)])
            } else {
                bodyRender = h('div', {
                    attrs: {
                        class: `${prefixClsCrm}-body`
                    }
                }, [
                    h('div', {
                        domProps: {
        ...
        computed: {
            iconTypeCls () {
                return [
                    `${prefixClsCrm}-head-icon`,
                    `${prefixClsCrm}-head-icon-${this.iconType}`
                ]
            },
            iconNameCls () {
                return [
...
BModal.SameInstance = (params = {}) => {
    const SameInstance = new Vue({
        data () {
            return {
                ...params,
                visible: false,
    ...
    const component = SameInstance.$mount()
    document.body.appendChild(component.$el)
    const modal = SameInstance.$children[0]
    return {
        show (props) {
            modal.$parent.showCancel = props.showCancel ? props.showCancel : true
            modal.$parent.iconType = props.icon
