    ...
    export default {
        ...
        props: {
            ...
            filterMethod: {
                type: Function,
                default (value, data) {
                    return data[this.defaultOpt.nameKey].indexOf(value) !== -1
                }
            },
            filterOptions: { // 模糊检索配置
                type: Object,
                default () {
                    return {
                        open: false // 是否开启检索后全部自动展开功能
                    }
                }
            },
            loadMethod: {
        ...
        created () {
            ...
            this.defaultRebuild('checked')
            this.defaultRebuild('expaned')
            this.filterTreeData(this.filterText, true)
        },
        mounted () {
        ...
        methods: {
            ...
            clearCheckedState (status) {
                this.dataList.forEach((item) => {
                    this.handleCheck({checked: status, nodeKey: item.nodeKey, isFormat: true})
                })
            },
            clearSelectedState () {
                ...
                if (arr.length) {
                    if (type === 'checked') {
                        if (this.showCheckbox) {
                            ...
                            this.handleSelect(node.nodeKey, true, true)
                        }
                    } else {
            ...
            // 单选
            handleSelect (nodeKey, isFormat = false, isTrue = false) {
                const selectedKey = this.defaultOpt.selectedKey
                ...
                let selectStatus = isTrue ? true : !node.selected
                this.$set(node, selectedKey, selectStatus)
            ...
            // 模糊检索
            filterTreeData (value, isReset = false) {
                const defaultOpt = this.defaultOpt
                ...
                const traverseVisible = function (data) {
                    ...
                    if (!isReset && childNodes.length && filterOptions.open) {
                        _this.$set(data, defaultOpt.expandKey, true)
                    }
